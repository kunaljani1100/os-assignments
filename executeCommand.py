...
def usePipeAndExecute(command, parameters, newenv):
	...
	# If the process is not a child process, then another child process is created from the first level child process after closing the write descriptor.
	if(pid != 0):
		try:
			os.close(w)
			os.waitpid(pid,0)
			...
			# If the process is not a child process, then it waits until the reading from pipe is completed.
			if(pid2 != 0):
				try:
					os.waitpid(pid2,0)
				except KeyboardInterrupt:
